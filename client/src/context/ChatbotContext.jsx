...
export const ChatbotProvider = ({ children }) => {
  ...
  const fetchConversations = async () => {
    try {
      const res = await authFetch('http://localhost:5000/api/chat/history');
      if (res.ok) {
  ...
  const loadConversation = async (id) => {
    try {
      const res = await authFetch(`http://localhost:5000/api/chat/history/${id}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const confirmPendingDelete = async (confirmed) => {
    ...
    if (confirmed) {
      try {
        const res = await authFetch('http://localhost:5000/api/chat/execute', {
          method: 'POST',
  ...
  const addMessage = async (text, sender = 'user', dataPayload = null, moduleName = null) => {
    ...
    try {
      ...
      if (activeId) {
        ...
        await authFetch(`http://localhost:5000/api/chat/history/${activeId}/messages`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
